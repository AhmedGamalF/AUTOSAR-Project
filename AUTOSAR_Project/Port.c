...
void Port_Init(const Port_ConfigType* ConfigPtr)
{
    ...
    if(FALSE== error)
    {
        ...
        for(Port_PinID = PORT_ZERO ; Port_PinID < PORT_CONFIGURED_PINS; Port_PinID++)
        {
            ...
            {
            case PORT_PIN_IN: CLEAR_BIT(*(volatile uint32 *)((volatile uint8 *)Ptr_Port + PORT_DIR_REG_OFFSET) , Port_PinPtr[Port_PinID].Pin_Num) ; /*  Set the PIN as an input*/

            /* Select the Pin Resistor */
            switch (Port_PinPtr[Port_PinID].Resistor)
            {
            case PULL_DOWN: CLEAR_BIT(*(volatile uint32 *)((volatile uint8 *)Ptr_Port + PORT_DIR_REG_OFFSET) , Port_PinPtr[Port_PinID].Pin_Num) ; /* Pull Down Resistor*/
            break;

            case PULL_UP: SET_BIT(*(volatile uint32 *)((volatile uint8 *)Ptr_Port + PORT_DIR_REG_OFFSET) , Port_PinPtr[Port_PinID].Pin_Num) ; /* Pull UP Resistor*/
            break;
            ...
            case PORT_PIN_OUT: SET_BIT(*(volatile uint32 *)((volatile uint8 *)Ptr_Port + PORT_DIR_REG_OFFSET) , Port_PinPtr[Port_PinID].Pin_Num) ; /* Set the pin as an output*/

            /* Select the Pin Level */
            switch (Port_PinPtr[Port_PinID].Initial_Level)
            {
            case PORT_PIN_LEVEL_LOW: CLEAR_BIT(*(volatile uint32 *)((volatile uint8 *)Ptr_Port + PORT_DIR_REG_OFFSET) , Port_PinPtr[Port_PinID].Pin_Num) ; /* PIN Level is Low */
            break;

            case PORT_PIN_LEVEL_HIGH: SET_BIT(*(volatile uint32 *)((volatile uint8 *)Ptr_Port + PORT_DIR_REG_OFFSET) , Port_PinPtr[Port_PinID].Pin_Num) ; /* PIN Level is High*/
            break;
